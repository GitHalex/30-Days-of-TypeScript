...
const patternBoolean = /love/;
const result = patternBoolean.test(str);
console.log(result);
const patternArray = /love/g;
const resultadoArr = str.match(patternArray);
if (resultadoArr) {
...
const txt = "Python es el lenguaje más bello que ha creado el ser humano.\
Recomiendo python para un primer lenguaje de programación";
let matchReplaced = txt.replace(/Python|python/, "JavaScript");
console.log(matchReplaced);
let matchReplacedGlobal = txt.replace(/Python|python/g, "JavaScript");
console.log(matchReplacedGlobal);
const txtEspecial = "%So%y p%r%%of%%es%or%a% y m%e %% enc%an%ta en%se%ña%r.\
...
let especial = txtEspecial.replace(/%/g, "");
console.log(especial);
const patternA_a = /[Aa]pple|[Bb]anana/g;
const txtString = "Apple and banana are fruits. An old cliche says an apple a day keeps the doctor away has been replaced by a banana a day keeps the doctor far far away.";
const apples = txtString.match(patternA_a);
if (apples) {
    console.log(apples);
}
else {
...
const patternDig = /\d+/g; // \d es un carácter especial que significa dígitos si agregamos + saca digitos esteros
const txtDig = "This regular expression example was made in January 12, 2020.";
const matches = txtDig.match(patternDig);
if (matches) {
...
const txtPunto = "Apple and banana are fruits";
const patternPunto = /[a].+/g; // ["an", "an", "an", "a ", "ar"] ['and banana are fruits']
const resultadoPutno = txtPunto.match(patternPunto);
if (resultadoPutno) {
...
const patternAsterisco = /[a].*/g; //. cualquier carácter, + cualquier carácter una o más veces
const resultadoAsterisco = txtPunto.match(patternAsterisco);
...
const textoSueldo = "Gana 4000 euros de sueldo al mes, 10000 euros de bonificación anual, 5500 euros de cursos online al mes";
...
const calcularIngresosAnuales = (texto) => {
    let totalAnual = 0;
    let match;
...
const textoA = "La posición de algunas partículas en el eje horizontal x -12, -4, -3 y -1 en la dirección negativa, 0 en el origen, 4 y 8 en la dirección positiva.";
...
const calcularDistanciaMasLejana = (texto) => {
    const posiciones = [];
    let match;
    // Recorremos todas las coincidencias encontradas por la expresión regular
...
console.log(`La distancia entre las dos partes más lejanas es: ${distanciaMasLejana}`);
const tenMostFrequentWords = (texto) => {
...
// Ejemplo de uso
const textoF = "This is a test. This test is only a test. If this had been an actual emergency, the test you just saw would have been followed by official information. This is only a test.";
...
const threeMostFrequentWords = (sentence) => {
